...
#import "CorEXTRuntimeExtensions.h"
#import <ctype.h>
...
typedef void (^ac_specialProtocolInjectionBlock)(Class);

// a `const char *` equivalent to system struct objc_method_description
typedef struct {
    SEL name;
    const char *types;
} ac_methodDescription;
 ...
 */
static void ac_injectSpecialProtocols (void) {
    /*
    ...
    // use this instead of ac_copyClassList() to avoid sending +initialize to
    ...
    @autoreleasepool {
        ...
        for (size_t i = 0;i < specialProtocolCount;++i) {
            ...
            // from the structure
            ac_specialProtocolInjectionBlock injectionBlock = (__bridge_transfer id)specialProtocols[i].injectionBlock;
...
unsigned ac_injectMethods (
                            Class aClass,
                            Method *methods,
                            unsigned count,
                            ac_methodInjectionBehavior behavior,
                            ac_failedMethodCallback failedToAddCallback
                            ) {
    ...
    @autoreleasepool {
        ...
        if (!isMeta) {
            ...
            behavior &= ~(ac_methodInjectionIgnoreLoad | ac_methodInjectionIgnoreInitialize);
        }
        ...
        for (unsigned methodIndex = 0;methodIndex < count;++methodIndex) {
            ...
            if (behavior & ac_methodInjectionIgnoreLoad) {
            ...
            if (behavior & ac_methodInjectionIgnoreInitialize) {
            ...
            switch (behavior & ac_methodInjectionOverwriteBehaviorMask) {
                case ac_methodInjectionFailOnExisting:
                    success = class_addMethod(aClass, methodName, impl, type);
                    break;
                    
                case ac_methodInjectionFailOnAnyExisting:
                ...
                case ac_methodInjectionReplace:
                    class_replaceMethod(aClass, methodName, impl, type);
                    break;
                    
                case ac_methodInjectionFailOnSuperclassExisting:
                {
...
BOOL ac_injectMethodsFromClass (
                                 Class srcClass,
                                 Class dstClass,
                                 ac_methodInjectionBehavior behavior,
                                 ac_failedMethodCallback failedToAddCallback)
{
    ...
    // ignore +load
    behavior |= ac_methodInjectionIgnoreLoad;
    addedCount = ac_injectMethods(
...
Class ac_classBeforeSuperclass (Class receiver, Class superclass) {
...
Class *ac_copyClassList (unsigned *count) {
...
unsigned ac_addMethods (Class aClass, Method *methods, unsigned count, BOOL checkSuperclasses, ac_failedMethodCallback failedToAddCallback) {
    ac_methodInjectionBehavior behavior = ac_methodInjectionFailOnExisting;
    if (checkSuperclasses)
        behavior |= ac_methodInjectionFailOnSuperclassExisting;
    
    return ac_injectMethods(
                             aClass,
                             methods,
                             count,
                             behavior,
...
BOOL ac_addMethodsFromClass (Class srcClass, Class dstClass, BOOL checkSuperclasses, ac_failedMethodCallback failedToAddCallback) {
    ac_methodInjectionBehavior behavior = ac_methodInjectionFailOnExisting;
    if (checkSuperclasses)
        behavior |= ac_methodInjectionFailOnSuperclassExisting;
    
    return ac_injectMethodsFromClass(srcClass, dstClass, behavior, failedToAddCallback);
}

BOOL ac_classIsKindOfClass (Class receiver, Class aClass) {
    while (receiver) {
        if (receiver == aClass)
            return YES;
...
Class *ac_copyClassListConformingToProtocol (Protocol *protocol, unsigned *count) {
    Class *allClasses;
    ...
    @autoreleasepool {
        ...
        allClasses = ac_copyClassList(&classCount);
...
ac_propertyAttributes *ac_copyPropertyAttributes (objc_property_t property) {
    ...
    ac_propertyAttributes *attributes = calloc(1, sizeof(ac_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate ac_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        switch (flag) {
            ...
            case 'C':
                attributes->memoryManagementPolicy = ac_propertyMemoryManagementPolicyCopy;
                break;
                
            case '&':
                attributes->memoryManagementPolicy = ac_propertyMemoryManagementPolicyRetain;
                break;
...
Class *ac_copySubclassList (Class targetClass, unsigned *subclassCount) {
    unsigned classCount = 0;
    Class *allClasses = ac_copyClassList(&classCount);
...
Method ac_getImmediateInstanceMethod (Class aClass, SEL aSelector) {
...
BOOL ac_getPropertyAccessorsForClass (objc_property_t property, Class aClass, Method *getter, Method *setter) {
    ac_propertyAttributes *attributes = ac_copyPropertyAttributes(property);
    if (!attributes)
        return NO;
...
NSMethodSignature *ac_globalMethodSignatureForSelector (SEL aSelector) {
    ...
    static ac_methodDescription volatile methodDescriptionCache[selectorCacheLength];
    ...
    ac_methodDescription methodDesc;
    ...
    methodDesc = (ac_methodDescription){.name = NULL, .types = NULL};
    
    uint classCount = 0;
    Class *classes = ac_copyClassList(&classCount);
    ...
    if (classes) {
        @autoreleasepool {
            ...
            for (uint i = 0;i < classCount;++i) {
                ...
                if (method) {
                    methodDesc = (ac_methodDescription){.name = aSelector, .types = method_getTypeEncoding(method)};
                    break;
                }
            }
        }
    ...
    if (!methodDesc.name) {
        ...
        if (protocols) {
            ...
            for (uint i = 0;i < protocolCount;++i) {
                ...
                if (objcMethodDesc.name) {
                    methodDesc = (ac_methodDescription){.name = objcMethodDesc.name, .types = objcMethodDesc.types};
                    break;
                }
            }
            free(protocols);
...
BOOL ac_loadSpecialProtocol (Protocol *protocol, void (^injectionBehavior)(Class destinationClass)) {
    @autoreleasepool {
        ...
        // ac_injectSpecialProtocols()
#ifndef __clang_analyzer__
        ac_specialProtocolInjectionBlock copiedBlock = [injectionBehavior copy];
...
void ac_specialProtocolReadyForInjection (Protocol *protocol) {
    @autoreleasepool {
        NSCParameterAssert(protocol != nil);
...
void ac_removeMethod (Class aClass, SEL methodName) {
    Method existingMethod = ac_getImmediateInstanceMethod(aClass, methodName);
    if (!existingMethod) {
        return;
    }
...
void ac_replaceMethods (Class aClass, Method *methods, unsigned count) {
    ac_injectMethods(
                      aClass,
                      methods,
                      count,
                      ac_methodInjectionReplace,
                      NULL
                      );
}

void ac_replaceMethodsFromClass (Class srcClass, Class dstClass) {
    ac_injectMethodsFromClass(srcClass, dstClass, ac_methodInjectionReplace, NULL);
}

NSString *ac_stringFromTypedBytes (const void *bytes, const char *encoding) {
    switch (*encoding) {
