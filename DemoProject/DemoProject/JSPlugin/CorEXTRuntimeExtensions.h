 ...
 */
typedef void (*ac_failedMethodCallback)(Class, Method);

/**
 * Used with #ac_injectMethods to determine injection behavior.
 */
typedef NS_OPTIONS(NSUInteger, ac_methodInjectionBehavior) {
    /**
     * Indicates that any existing methods on the destination class should be
     * overwritten.
     */
    ac_methodInjectionReplace                  = 0x00,
    
    /**
     * Avoid overwriting methods on the immediate destination class.
     */
    ac_methodInjectionFailOnExisting           = 0x01,
    
    /**
     * Avoid overriding methods implemented in any superclass of the destination
     * class.
     */
    ac_methodInjectionFailOnSuperclassExisting = 0x02,
    
    /**
     * Avoid overwriting methods implemented in the immediate destination class
     * or any superclass. This is equivalent to
     * <tt>ac_methodInjectionFailOnExisting | ac_methodInjectionFailOnSuperclassExisting</tt>.
     */
    ac_methodInjectionFailOnAnyExisting        = 0x03,
    
    /**
     * Ignore the \c +load class method. This does not affect instance method
     * injection.
     */
    ac_methodInjectionIgnoreLoad = 1U << 2,
    
    /**
     * Ignore the \c +initialize class method. This does not affect instance method
     * injection.
     */
    ac_methodInjectionIgnoreInitialize = 1U << 3
};

/**
 * A mask for the overwriting behavior flags of #ac_methodInjectionBehavior.
 */
static const ac_methodInjectionBehavior ac_methodInjectionOverwriteBehaviorMask = 0x3;
...
typedef enum {
     ...
     */
    ac_propertyMemoryManagementPolicyAssign = 0,
    
    /**
     * The value is retained.
     */
    ac_propertyMemoryManagementPolicyRetain,
    
    /**
     * The value is copied.
     */
    ac_propertyMemoryManagementPolicyCopy
} ac_propertyMemoryManagementPolicy;
...
typedef struct {
     ...
     * #ac_propertyMemoryManagementPolicyAssign if #readonly is \c YES.
     */
    ac_propertyMemoryManagementPolicy memoryManagementPolicy;
 ...
 */
Class *ac_copySubclassList (Class aClass, unsigned *subclassCount);
 ...
 */
#define ac_getIvar(OBJ, IVAR, TYPE) \
 ...
 * or floating-point types.
 */
#define ac_getIvarByName(OBJ, NAME, TYPE) \
ac_getIvar((OBJ), class_getInstanceVariable(object_getClass((OBJ)), (NAME)), TYPE)
 ...
 */
unsigned ac_injectMethods (Class aClass, Method *methods, unsigned count, ac_methodInjectionBehavior behavior, ac_failedMethodCallback failedToAddCallback);

/**
 * Invokes #ac_injectMethods with the instance methods and class methods from
 * \a srcClass. #ac_methodInjectionIgnoreLoad is added to #behavior for class
 * method injection.
 *
 * Returns whether all methods were added successfully. For each method that fails
 * to be added, \a failedToAddCallback (if provided) is invoked.
 ...
 */
BOOL ac_injectMethodsFromClass (Class srcClass, Class dstClass, ac_methodInjectionBehavior behavior, ac_failedMethodCallback failedToAddCallback);
 ...
 *
 * @li Each protocol is loaded with #ac_loadSpecialProtocol and a custom block
 ...
 * #ac_specialProtocolReadyForInjection.
 ...
 * #ac_specialProtocolReadyForInjection as well.
 *
